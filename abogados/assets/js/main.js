/* Rivera & Torres Abogados — main.js */
(function () {
  'use strict';
  ...
  (function agendaModule() {
    ...
    var occupied = { 1:[0,3,5], 2:[1,4], 3:[2,5], 4:[0,3], 5:[1,4], 6:[0,2] };
    ...
    function genSlots(dow, durMin) {
      ...
      if (!hours) return [];
      var slots = [], cur = hours[0]*60, end = hours[1]*60;
      var occ = occupied[dow] || [], i = 0;
      while (cur + durMin <= end) {
        var h = Math.floor(cur/60), m = cur%60;
        slots.push({ label:(h<10?'0':'')+h+':'+(m===0?'00':m), available:occ.indexOf(i)===-1 });
        cur += durMin; i++;
      }
      return slots;
    }
    ...
    function renderSlots(slots) {
      ...
      slots.forEach(function (s) {
        ...
        if (!s.available) {
          btn.disabled = true;
          btn.setAttribute('aria-label', s.label + ' — no disponible');
        } else {
    ...
    selSvc.addEventListener('change', function () {
      ...
      var parts = val.split('|');
      state.durMin = parseInt(parts[0], 10); state.svcLabel = parts[1];
      state.fecha = null; state.slotTime = null;
      selFecha.value = ''; slotsGrid.innerHTML = '';
      hide(slotsEmpty); hide(stepSlots); hide(stepDatos); hide(stepConf);
      show(stepFecha); updateResumen();
      stepFecha.scrollIntoView({ behavior:'smooth', block:'nearest' });
    });

    /* Paso 2: fecha */
    selFecha.addEventListener('change', function () {
      var val = selFecha.value; if (!val) return;
      var d = new Date(val+'T00:00:00'), dow = d.getDay();
      if (!schedule[dow]) {
        slotsGrid.innerHTML = '';
        slotsEmpty.textContent = 'Los domingos el estudio está cerrado. Elija otro día.';
        show(slotsEmpty); show(stepSlots); hide(stepDatos); return;
      }
      state.fecha = val; state.slotTime = null; hide(stepDatos);
      renderSlots(genSlots(dow, state.durMin));
      show(stepSlots); updateResumen();
    ...
    btnConf && btnConf.addEventListener('click', function () {
      ...
      if (!nombre || !email || !tel) {
        formError.textContent = 'Por favor complete nombre, correo y teléfono para continuar.';
        show(formError); return;
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        formError.textContent = 'El correo electrónico no es válido. Por favor revíselo.';
        show(formError); return;
      }
      hide(formError);
      /* PRODUCCIÓN: fetch('/api/consultas', { method:'POST', body: JSON.stringify({...}) }) */
      confirmRes.textContent =
        nombre + ', su consulta sobre ' + state.svcLabel +
        ' está reservada para el ' + formatFecha(state.fecha) + ' a las ' + state.slotTime +
        '. Nos comunicaremos con usted para confirmar los detalles.';
      hide(stepSlots); hide(stepDatos); hide(resumenBar);
      show(stepConf);
      stepConf.scrollIntoView({ behavior:'smooth', block:'center' });
      // window.dataLayer = window.dataLayer || [];
      // window.dataLayer.push({ event: 'consulta_reservada', tipo: state.svcLabel });
    });

    /* Reset */
    btnReset && btnReset.addEventListener('click', function () {
      selSvc.value = '';
      ['ag-nombre','ag-email','ag-tel','ag-caso'].forEach(function (id) {
